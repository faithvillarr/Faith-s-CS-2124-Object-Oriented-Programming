...
public:
    ...
    // ???
    // ...
    Directory(const string& name): company(name), size(0), capacity(4), entries(new Entry *[4]){};


    //Copy
    Directory(const Directory& d): capacity(d.capacity), size(d.size), company(d.company){
    ...
    void add(const string& name, unsigned room, unsigned ph, Position& pos) {
        if (size == capacity)	{
            ...
            Entry** new_arr = new Entry*[capacity];
            for (size_t i = 0; i < size; i ++){
                new_arr[i] = entries[i];
...
private:
    Entry** entries;// = new Entry*[capacity]; // Notice the type!!! Pointer to Entry pointers.
    size_t size;
    size_t capacity;
    string company;
}; // class Directory
